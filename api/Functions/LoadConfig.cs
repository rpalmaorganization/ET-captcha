...
public class LoadConfig
{
            ...
            HttpRequestData req)
    {
        ...
        string? test_preview = Environment.GetEnvironmentVariable("TEST_PREVIEW");
        IDictionary env_vars1 = Environment.GetEnvironmentVariables(EnvironmentVariableTarget.Process);
        IDictionary env_vars2 = Environment.GetEnvironmentVariables(EnvironmentVariableTarget.Machine);
        IDictionary env_vars3 = Environment.GetEnvironmentVariables(EnvironmentVariableTarget.User);


#if IN_PREVIEW
        Console.WriteLine("Internal build specific code is running.");
        _landing_options.SiteKey = _landing_options.TestSiteKey;
        _landing_options.SecretKey = _landing_options.TestSecretKey;
#else
        Console.WriteLine("Standard build code is running.");
#endif
